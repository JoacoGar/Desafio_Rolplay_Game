public class Mago : IPersonaje
{
    ...
    public Mago(string nombre)
    {
        ...
        Puntos_de_salud = 100;
        Puntos_de_ataque =
            0; // Declaro unos puntos de ataque y de defensa iniciales en 0, dado que los mismos seran otorgados por los distintos elementos.
        Puntos_de_defensa = 0;
    ...
    public void AtacarConHechizo(Mago personajeQueAtaca, IPersonaje personajeAtacado, string nombreHechizo)
    {
        for (int i = 0; i < Libro.LibroHechizos.Count; i++)
        {
            if (Libro.LibroHechizos[i].Nombre_elemento == nombreHechizo)
            {
                ...
                Console.WriteLine($"{personajeQueAtaca.Nombre} ataca a {personajeAtacado.Nombre} con un hechizo");
                Console.WriteLine(
                    $"{personajeAtacado.Nombre}: Puntos de salud: {personajeAtacado.Puntos_de_salud}, Puntos de defensa: {personajeAtacado.Puntos_de_defensa}, Puntos de ataque: {personajeAtacado.Puntos_de_ataque} ");
